...
- Canvas は `375 x 500` の内部座標系を維持する。
- 画面が低い端末でも上寄せにせず、Canvas、メッセージ、操作UIを中央に寄せる。
