...
- 基本は 41×41 グリッド迷路（再帰バックトラッカー生成）だが、序盤 1〜3F は 17×17 / 23×23 / 29×29 に縮小
...
- グリッドは奇数サイズ。基準サイズは `COLS=41, ROWS=41`
- `floor_size(floor)` で 1F=17×17、2F=23×23、3F=29×29、4F以降=41×41 を返す
- セル座標は偶数インデックスのみパス候補
- 壁（1）とパス（0）の二値マップ
...
- フロア遷移用シードは `wrapping_mul` を使って生成し、debug build の整数オーバーフロー panic を避ける
...
- ぷにこんは倒したベクトルを 4 方向へ量子化し、連続移動入力を送る
...
- タイトル: ハイスコアランキング + 「ゲームを始める」ボタン + build 短縮ハッシュ表示
