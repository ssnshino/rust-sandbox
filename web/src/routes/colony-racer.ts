...
const aircraftCatalog = {
  version: "colony-racer-aircraft-v1",
...
export async function registerColonyRacerRoutes(app: FastifyInstance) {
  app.get("/api/colony-racer/round-index", async (_request, reply) => {
  ...
  app.get<{ Params: { round: string } }>("/api/colony-racer/course/:round", async (request, reply) => {
    const rounds = await loadRounds();
  ...
  app.get("/api/colony-racer/field-catalog", async (_request, reply) => {
    const catalogPath = path.join(config.colonyRacerRoot, "StreamingAssets/AirRace/airrace3d_field.json");
    const fallback = { version: "colony-racer-field-v1", globalField: { minX: -32000, maxX: 32000, minZ: -32000, maxZ: 32000, margin: 640 }, overrides: [] };
    return sendNoStoreJson(reply, await readJsonOrFallback(catalogPath, fallback));
  });

  app.get("/api/colony-racer/world-object-catalog", async (_request, reply) => {
    const catalogPath = path.join(config.colonyRacerRoot, "StreamingAssets/AirRace/world_object_catalog.json");
    return sendNoStoreJson(reply, await readJsonOrFallback(catalogPath, { version: "colony-racer-world-v1", objects: [] }));
  });

  app.get("/api/colony-racer/round-world-catalog", async (_request, reply) => {
    return sendCatalog(reply, "round_world_catalog.json");
  });

  app.get("/api/colony-racer/aircraft-catalog", async (_request, reply) => {
    const catalogPath = path.join(config.colonyRacerRoot, "StreamingAssets/AirRace/aircraft_catalog.json");
    return sendNoStoreJson(reply, await readJsonOrFallback(catalogPath, aircraftCatalog));
  });

  app.get("/api/colony-racer/aircraft-prefab-catalog", async (_request, reply) => {
    return sendCatalog(reply, "aircraft_prefab_catalog.json");
  });

  app.get("/colony-racer", async (_request, reply) => sendColonyRacerFile(reply, "index.html"));
  app.get("/colony-racer/", async (_request, reply) => sendColonyRacerFile(reply, "index.html"));
  app.get<{ Params: { "*": string } }>("/colony-racer/*", async (request, reply) => {
    const requestPath = request.params["*"] || "index.html";
    return sendColonyRacerFile(reply, requestPath);
  });
}

async function sendCatalog(reply: FastifyReply, filename: string) {
  const catalogPath = path.join(config.colonyRacerRoot, "StreamingAssets/AirRace", filename);
  try {
...
async function sendColonyRacerFile(reply: FastifyReply, requestPath: string) {
  const fullPath = safeJoin(config.colonyRacerRoot, requestPath);
...
async function loadRounds(): Promise<Record<string, unknown>> {
  const publicRoundsPath = path.join(config.colonyRacerRoot, "StreamingAssets/AirRace/airrace_rounds.json");
  try {
